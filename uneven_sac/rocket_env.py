"""
Gymnasium-compatible Rocket Landing Environment for SAC with Uneven Terrain
...
class RocketLandingEnv(gym.Env):
    """
    Continuous action space rocket landing environment for SAC with uneven terrain.
    ...
    def __init__(self, max_steps=1000, task='landing', rocket_type='starship',
                 ...
                 enable_terrain=True, terrain_difficulty='moderate'):
        super().__init__()
    ...
    def reset(self, seed=None, options=None):
        ...
        # Regenerate terrain for each episode
        if self.enable_terrain:
            self.terrain = self.generate_terrain(difficulty=self.terrain_difficulty)
            target_height = self.get_terrain_height(0)
            self.target_y = target_height + self.H/2.0
    ...
    def step(self, action):
        ...
        info = {
            ...
            'terrain_height': self.get_terrain_height(x_new)
        }
    ...
    def check_crash(self, state):
        if self.task == 'hover':
            ...
            ground_level = terrain_height + self.H / 2.0
            
            crash = False
            if y >= self.world_y_max - self.H / 2.0:
                crash = True
            if y <= ground_level and v >= 15.0:
                crash = True
            if y <= ground_level and abs(x) >= self.target_r:
                crash = True
            if y <= ground_level and abs(theta) >= 10/180*np.pi:
                crash = True
            if y <= ground_level and abs(vtheta) >= 10/180*np.pi:
                crash = True
            return crash
    ...
    def check_landing_success(self, state):
        if self.task == 'hover':
            ...
            return True if y <= ground_level and v < 15.0 and abs(x) < self.target_r \
